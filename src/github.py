...
class GithubAPI(object):
    ...
    def process_issue(self, json_data):
        return self.process_issue_data(json_data['issue'])

    def process_issue_data(self, json_data):
        return GithubAPIIssue(json_data)
    ...
    def get_issue(self, username, repo, number, optional_params={}):
        return self.api_call(
            url='http://github.com/api/v2/json/issues/show/%s/%s/%s' % (username, repo, number),
            processor=self.process_issue,
            optional_params=optional_params
        )
...
class GithubAPIIssue(GenericAPIObject):
    pieces = ['number', 'votes', 'created_at', 'body', 'title', 'updated_at',
              'closed_at', 'user', 'labels', 'state']
